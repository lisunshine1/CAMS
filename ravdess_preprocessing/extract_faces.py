...
#
# failed_videos = []
# root = 'E:\\雷导研究生\\代码\\未改动\\更换数据据\\mer\\flv_to_mp4'
#
...
failed_videos = []
root = 'C:\yk\mul_cremad\cremad\CREMA-D\Video'
...
for filename in tqdm(sorted(os.listdir(root))):
    if filename.endswith('.mp4'):
        ...
        save_fps = save_frames // (framen // input_fps)
        if save_avi:
            out = cv2.VideoWriter(os.path.join(root, filename[:-4] + '_facecroppad.avi'),
        ...
        if save_avi:
            out.release()
        np.save(os.path.join(root, filename[:-4] + '_facecroppad.npy'), np.array(numpy_video))
        if len(numpy_video) != 15:
