...
# class Attention(nn.Module):
#     def __init__(self, in_dim_k, in_dim_q, out_dim, num_heads, qkv_bias=False, qk_scale=None, attn_drop=0.,
#                  proj_drop=0.):
#         super().__init__()
#         self.num_heads = num_heads
#         head_dim = out_dim // num_heads
...
#         self.proj_drop = nn.Dropout(proj_drop)
#         self.qkmatrix = None
#         self.extra_linear = nn.Linear(out_dim, out_dim)  # Extra linear layer
#
#     def forward(self, x, x_q):
#         B, Nk, Ck = x.shape
#         B, Nq, Cq = x_q.shape
...
#         x = self.proj(x)
#         x = self.proj_drop(x)
#         x = self.extra_linear(x)  # Apply extra linear layer
#
#         return x, self.qkmatrix
#
#
...
#                        use_conv1=use_conv1)
#         self.extra_mlp = Mlp(in_features=out_dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
#                              use_conv1=use_conv1)  # Extra MLP layer
#
#     def forward(self, xk, xq):
#         x, a = self.attn(self.norm1_k(xk), self.norm1_q(xq))
#         x = self.drop_path(x)
#         x = x + self.drop_path(self.mlp(self.norm2(x)))
#         x = x + self.drop_path(self.extra_mlp(self.norm2(x)))  # Apply extra MLP layer
#         return x
#
